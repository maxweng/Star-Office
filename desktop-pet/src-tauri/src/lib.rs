...
fn read_state_via_backend() -> Result<PetState, String> {
    let mut stream = std::net::TcpStream::connect("127.0.0.1:18790")
...
fn spawn_backend(root: &PathBuf) -> Option<Child> {
    if std::net::TcpStream::connect("127.0.0.1:18790").is_ok() {
        eprintln!("ℹ️ backend already running on 127.0.0.1:18790");
        return None;
    }
